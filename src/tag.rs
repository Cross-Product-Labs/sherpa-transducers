...
/// Configuration for [Tagging]. See [Tagging::from_pretrained] for a simple way to get started.
...
impl TaggingConfig {
    /// Make a [TaggingConfig] for a zipformer model with reasonable defaults.
    pub fn zipformer(model: &str, labels: &str) -> Self {
        Self::new(Model::Zipformer { model: model.into() }, labels)
    }

    /// Make a [TaggingConfig] for a consistent ensemble distillation model with reasonable defaults.
    pub fn ced(model: &str, labels: &str) -> Self {
        Self::new(Model::Ced { model: model.into() }, labels)
    }

    fn new(model: Model, labels: &str) -> Self {
        Self {
            model,
    ...
    /// Build your very own [Tagging].
    pub fn build(self) -> Result<Tagging> {
        ...
        match self.model {
            Model::Zipformer { model } => config.model.zipformer.model = track_cstr(dcs, &model),
            Model::Ced { model } => config.model.ced = track_cstr(dcs, &model),
        }
...
impl Tagging {
    ...
    pub async fn from_pretrained<S: AsRef<str>>(model: S) -> Result<TaggingConfig> {
        ...
        let Config { kind, arch } = serde_json::from_str(&config)?;
        ensure!(
        ...
        match arch.as_str() {
            "zipformer" => Ok(TaggingConfig::zipformer(
            ...
            "ced" => Ok(TaggingConfig::zipformer(
                repo.get("model.onnx").await?.to_str().unwrap(),
                repo.get("class_labels_indices.csv")
                    .await?
                    .to_str()
